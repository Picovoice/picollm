            ...
            using (PicoLLM picoLLM = PicoLLM.Create(accessKey, modelPath, device))
            {
                ...
                Action<float> progressCallback = (float progress) =>
                {
                    int bar_width = Math.Max(10,
                                        ...
                                        - new string("] 100.0% (press `Space` to interrupt)").Length
                                        - 1);
                    ...
                    if (progress >= 100.0f) {
                        ...
                        Console.Write($"] {progress:F1} (press `Space` to interrupt)");
                    }
                ...
                completion = picoLLM.GenerateOCR(image, completionTokenLimit, streamCallback, progressCallback);
                ...
                Console.WriteLine($"Total time elapsed\t{totalElapsedSec:F2} seconds");
            }
        }
