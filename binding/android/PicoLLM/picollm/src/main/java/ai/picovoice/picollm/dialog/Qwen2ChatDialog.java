...
/**
 * Represents a dialog helper specific for the `qwen2` model.
 */
public class Qwen2ChatDialog extends PicoLLMDialog {

    /**
     * Builder class for constructing Qwen2ChatDialog instances.
     */
    public static class Builder extends PicoLLMDialog.Builder {
        /**
         * Builds a new instance of Qwen2ChatDialog based on the configured settings.
         *
         * @return A new instance of Qwen2ChatDialog.
         */
        public Qwen2ChatDialog build() {
            return new Qwen2ChatDialog(this.history, this.system);
        }
    }

    /**
     * Constructs a Qwen2ChatDialog instance with the specified history and system settings.
     *
     * @param history The history length for the dialog.
     * @param system  The system instruction for configuring the model's responses.
     */
    Qwen2ChatDialog(Integer history, String system) {
    ...
    public String getPrompt() throws PicoLLMException {
        ...
        if (this.system != null) {
            res.append(String.format("<|im_start|>system\n%s<|im_end|>\n", this.system.trim()));
        }
        ...
        for (int i = 0; i < llm.size(); i++) {
            res.append(String.format(
                    "<|im_start|>user\n%s<|im_end|>\n",
                    human.get(i).trim()));
            res.append(String.format("<|im_start|>assistant\n%s<|im_end|>\n",
                    llm.get(i).trim()));
        }
